...
ใช้ shadcn `<Spinner>` ที่ install แล้ว — มี `role="status"` + `aria-label="Loading"` built-in:

```tsx
import { Spinner } from '@/components/ui/spinner';

<Spinner className="size-4 text-accent" />        {/* inline */}
<Spinner className="size-6" />                    {/* button + label */}
```

ใช้สำหรับ inline action (submit button, refresh icon, autocomplete) เท่านั้น — ห้ามเป็น page-level fallback (ใช้ skeleton แทน)

> รายละเอียดเพิ่มเติม + เมื่อไหร่ใช้ skeleton vs spinner: ดู `.claude/rules/loading-states.md`
